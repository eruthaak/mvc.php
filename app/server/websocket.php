...
const WS_HOST = '192.168.1.191'; // Host
...
while (true) { // Start endless loop, so that our script doesn't stop
	...
	if (in_array($socket, $changed)) /* Check for new socket */ {
		...
		if(WS_DEBUG) echo "New Connection: ($ip) Connected.\n";
	...
	foreach ($changed as $changedSocket) {	// Loop through all connected sockets
		while(@socket_recv($changedSocket, $buf, 1024, 0) >= 1) { // Check for any incomming data
			...
			$response_text = mask($data); // Prepare data to be sent to client
			echo "($ip): $data\n";
		...
		if ($buf === false) { // Check disconnected client
			...
			if(WS_DEBUG) echo "Lost Connection: ($ip) Disconnected.\n";
...
function handshake($receivedHeader, $clientConnection, $host = WS_HOST, $port = WS_PORT) {
	$headers = array();
	$lines = preg_split("/\r\n/", $receivedHeader);
